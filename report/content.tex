...
The workflow depends on hostname to IP resolution. First, it checks whether the index data from the last run can be reused. This is done by checking whether the cluster size stayed the same. If not, each node creates a new Elasticsearch instance in a temporary directory. Then, using \texttt{MPI\_GATHER}, a list of tuples \texttt{(rank, hostname)\ is collected at \ac{MPI} rank 0, i.e. the root process. After this, the root creates or updates a config for each node depending on whether they were reused, while the rest of the nodes wait. Rank 0 (or 0/1/2 for larger clusters) are always master eligable nodes, and every node has the data role. An autogenerated example configuration can be found in the Appendix. Lastly, each rank starts the Singularity container with the new config and the previously created Elasticsearch bind-mounted in.
